...
namespace gscam {
  ...
  bool GSCam::configure()
  {
    ...
    gsconfig_env = "v4l2src device=/dev/video2 ! video/x-raw-rgb,framerate=30/1,width=640,height=480 ! ffmpegcolorspace";
  ...
  bool GSCam::init_stream()
  {
    ...
    if (image_encoding_ == "jpeg") {
        jpeg_pub_ = nh_.advertise<sensor_msgs::CompressedImage>("right_camera/image_raw/compressed",1);
        cinfo_pub_ = nh_.advertise<sensor_msgs::CameraInfo>("right_camera/camera_info",1);
    } else {
        camera_pub_ = image_transport_.advertiseCamera("right_camera/image_raw", 1);
    }
